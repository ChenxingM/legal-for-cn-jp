...
| `jp-subcontract-review` | 下請取引 (B2B 外注) 的下請法合规审计：親-下請事業者 资本阈值判定、第3条 8 项书面必备事项、60 日支付上限、第4条 11 项禁止行为、14.6%/年遅延利息、2 年书类保存义务 |
| `jp-tax-qa` | **新增 v0.3.0**。日本税法 Q&A：所得税法 / 法人税法 / 消費税法（含 インボイス制度）/ 相続税法 / 印紙税法 / 租税特別措置法 等 10 部。通过 `egov_client.py` 动态从 e-Gov 拉取条文，输入语言匹配，缺信息（個人/法人・課税事業者状態 等）会主动追问 |
...
| `cn-tax-qa` | **新增 v0.3.0**。中国税法 Q&A：个人所得税 / 企业所得税 / 增值税（2024-12-25 新法）/ 印花税 / 关税 / 税收征收管理法 等 15 部，由 LawRefBook 周同步。输入语言匹配，缺信息（居民/非居民・一般/小规模 等）会主动追问 |
| `cn-law-lookup` | 中国法条查询——内置 21 部核心法律。长尾暂未覆盖（计划后续覆盖更多法律部门） |
| `cross-border-tax` | **新增 v0.3.0**。中日跨境税务专项：中日租税条約（1983 / 议定书）、源泉徴収 vs 预提所得税、移転価格、跨境ライセンス料 / 配信権 / 出演料 课税、PE 認定。每次同时引用双边条文 + 条約条款 |
...
`/plugin list` 应能看到 `legal-for-cn-jp` 与 10 个 skill。
...
**日本税法（v0.3.0 起）**：10 部主要税法（所得税法 / 法人税法 / 消費税法 / 相続税法 / 国税通則法 / 国税徴収法 / 印紙税法 / 租税特別措置法 / 関税法 / 地方税法）**不本地内置**——通过 `egov_client.py fetch <法令ID>` 动态从 e-Gov API 拉取（每个法都几百条几百 KB；动态拉取始终最新，避免巨大仓库）。法令ID 表内置在 `jp-tax-qa` skill 里。

**中国侧**：21 部核心法律（均为"法律"层级，由全国人大及其常委会制定），清洗后的 Markdown（约 0.97 MB），存放在 `references/cn_laws/`。**不包含**：宪法、行政法规（国务院条例）、地方性法规（省级人大）、监察法规（国家监察委）、司法解释（最高法／最高检）、部门规章（部委级办法）。这些层级是 v0.4.0+ 的扩展计划（详见 ROADMAP）。

**中国税法（v0.3.0 起）**：15 部税收法律由 GitHub Actions 周同步 LawRefBook：个人所得税法、企业所得税法、增值税法（2024-12-25 新法）、印花税法、关税法、税收征收管理法、资源税法、城市维护建设税法、契税法、环境保护税法、耕地占用税法、烟叶税法、船舶吨税法、车船税法、车辆购置税法。**不包含**：实施条例 / 财政部 + 国家税务总局 公告（动画行业税收优惠 "真答案" 所在 — v0.4.0+）。
...
- **v0.3.1 / v0.4.0**：税法长尾——JP 国税庁通達/告示、CN 财政部+国家税务总局公告/实施条例（动画行业税收优惠的"真答案"在这层）
- **v0.4.0**：判例检索（先日本后中国——判例／裁判文书）+ 中国法长尾（行政法规／司法解释／地方性法规等）
- **v0.5.0**：日中英三语法律术语对照表、动画行业合同模板
- **v0.6.0**：AI 立法专项（生成式AI暂行办法、広島AIプロセス、EU AI Act 对照）

## 局限性——务必阅读

1. **不构成法律意见。** 任何输出在被采纳前都需经持牌律师审核。
2. **只覆盖成文法，不覆盖判例。** 判例（日本）和裁判文书（中国）计划在 v0.4.0 加入。
...
5. **中国侧只覆盖"法律"层级。** 当前 21 部均为全国人大及其常委会制定的"法律"，**不含**：宪法 / 行政法规（国务院条例）/ 地方性法规（省级人大）/ 监察法规（国家监察委）/ 司法解释（最高法、最高检）/ 部门规章（部委级办法）。这些层级将 v0.3.0+ 通过扩展 `LRB_SOURCED_LAWS` 或换更全的上游逐步覆盖。
...
- 中国核心法语料：21 部里有 17 部由 [LawRefBook/Laws](https://github.com/LawRefBook/Laws) 同步；4 部（民法典 / 反不正当竞争法 / 网络安全法 / 仲裁法）由维护者从 [flk.npc.gov.cn 国家法律法规数据库](https://flk.npc.gov.cn/) 获取的 .docx 通过 `refresh_cn_corpus.py` 处理。
