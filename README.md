...
**中国侧**：21 部核心法律（均为"法律"层级，由全国人大及其常委会制定），清洗后的 Markdown（约 0.97 MB），存放在 `references/cn_laws/`。**不包含**：宪法、行政法规（国务院条例）、地方性法规（省级人大）、监察法规（国家监察委）、司法解释（最高法／最高检）、部门规章（部委级办法）。这些层级是 v0.3.0+ 的扩展计划（详见 ROADMAP）。
